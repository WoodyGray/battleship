...
import org.woody.game.GameStatistics;
...
public class BattleshipGameServerEndpoint {
    ...
    public static void startServer(int port, BattleController battleController) {
        ...
        try {
            server.start();
            System.out.println("---сервер запущен, ждем игрока");
            while (!isFinished) {
                Thread.onSpinWait();
            }
        } catch (Exception e) {
